...
{
    $did = $_GET['did'];
    $did1 = mysqli_query($connection, "delete from tbl_bookmark where bookmark_id  ='{$did}'") or die(mysqli_error($connection));
}
?>
<!DOCTYPE HTML>
<html class="no-js">
...
<div class="body">
  ...
  <div class="main" role="main">
          		...
          		<div class="page">
        			<div class="row">
                  		...
                  		<div class="block-heading" id="details">
                         		...
                         		<h4><span class="heading-icon"><i class="fa fa-home"></i></span>Bookmarked PGs'</h4>
                     		</div>
    ...
    `tbl_bookmark`.`bookmark_id`
    , `tbl_pg_master`.`pg_name`
    , `tbl_pg_master`.`pg_monthlycharge`
    , `tbl_bookmark`.`user_id`
FROM
    `tbl_pg_master`
    INNER JOIN `tbl_bookmark` 
        ON (`tbl_pg_master`.`pg_id` = `tbl_bookmark`.`pg_id`) where `tbl_bookmark`.`user_id` = '{$_SESSION['user_id']}'") or die(mysqli_error($connection));
                                    while($data = mysqli_fetch_array($q))
                                    {
                                        echo "<tr>
                                        
                                        <td><a href='property-detail.html'>{$data[1]}</a></td>
                                       
                                        <td><div class='price'><strong>Rs</strong><span>Rs.{$data[2]} </span></div></td>
                                        <td>
                                            
                                            <a href='#' class='action-button'><i class='fa fa-ban'></i> <span><a href='?did=$data[0]'>Remove</a></span></a>                                        </td>
                                    </tr>";
                                    }
                                    ?>
